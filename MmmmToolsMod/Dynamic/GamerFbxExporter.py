...
class GamerFbxExporter(object):
    ...
    def getPreset(self, presetName = 'mmmmDefault.fbxexportpreset', presetFolder = None):
        ...
        if presetFolder is None:
            presetFolder = presetFolderBase + '/assets/presets/fbx_export_presets/'
            
        presetStr=presetFolder+presetName
        
        return presetStr
    def getCurrentScenePath( self ):        scnPathAndFilename = cmds.file(q=True, sceneName=True)        scnPathAndFilename = scnPathAndFilename.replace('\\','/')        assert( not scnPathAndFilename.endswith('/') )        spl = scnPathAndFilename.split('/')        spl.pop()        scnPathNoFilename = '/'.join( spl )        return scnPathNoFilename        
    def addAttributesForExport(self):
        self.addAttributeForExportFile()
        self.addAttributeForExportPath()        self.addAttributeForExportPreset()
    ...
    def addAttributeForExportPath(self):
        for obj in pm.ls(selection=True, transforms=True):
            try:
                ...
                print( "Attribute not added, it might already exist on the selected object.")
                print traceback.format_exc()
    def addAttributeForExportPreset(self):
        for obj in pm.ls(selection=True, transforms=True):
            try:
                obj.addAttr( "mmmmExportPreset", dataType="string" )  #longname="
            except:
                print( "Attribute not added, it might already exist on the selected object.")
                print traceback.format_exc()
