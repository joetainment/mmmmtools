...
from MmmmToolsMod.unipath.abstractpath import AbstractPath
from MmmmToolsMod.unipath.errors import RecursionError
...
class Path(AbstractPath):
    ...
    def absolute(self):
        ...
        if not self.relative_to is None:
            try:
                return self.__class__( self.relative_to.absolute(), self )
            except:  ## catch it if it was a string
                assert (isinstance( self.relative_to, str ) or 
                    isinstance( self.relative_to, unicode ) or
                    isinstance( self.relative_to, AbstractPath ) )
                return self.__class__(self.relative_to + self)
        else:
    ...
    def relative(self):
        ...
        if self.isabsolute():
            if self.startswith( self.relative_to.absolute()  ):
                new_relative_path = \
                    self.__class__(
                        self.replaceInPath( self.relative_to.absolute(), ''),
            ...
            return self.__class__( self.relative_to.rel_path_to(self) )
    
    def replaceInPath(self, old_text, new_text):
        if not old_text.endswith( os.sep ):
            old_text = old_text + os.sep
        return self.__class__( self.replace( old_text, new_text )  )
    
    def fix_path_to_end_in_sep(self):
        if not (self.endswith( os.sep ) and len(self)>0):
            return Path( self, os.sep, relative_to = self.relative_to )
        else:
            return self
